  ...
  <div id="uterm" class="console" :class="{'open': open, 'closed': !open, 'sm': (ss == 35), 'x-sm': (ss == 25), 'xx-sm': (ss == 15)}">
    <div class="console-inner">
      ...
      <div class="uwrapper">
        ...
        <div class="cli-input">
          <span class="cyan">observer</span>@{{r}}:<span class="jaundice">~/{{i}}</span><span> $ </span>
          <input 
            type="search" 
            id="cli"
            class="input" 
...
export default {
  ...
  methods: {
     ...
     */
    cp: function (cmd) {
      this.log.push('<span class="cyan">observer</span>@' + this.r + ':<span class="jaundice">~/' + String(this.i) + '</span><span> $ </span> ' + cmd);
