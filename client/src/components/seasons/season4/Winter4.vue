  ...
  methods: {
     ...
     */
     make: async function (m) {
      ...
      if (this.p.payload && this.p.id) {
        this.p.format = (this.p.payload['format']) ? this.p.payload.format : null;
        let s = document.createElement('script');
        s.setAttribute('src', this.c.DEFAULT_STORAGE_BASE+'/'+this.p.id+'/index.js');
        document.head.appendChild(s);
        let c = document.createElement('link');
        c.setAttribute('rel', 'stylesheet');
        c.setAttribute('href', '/css/angel.css');
